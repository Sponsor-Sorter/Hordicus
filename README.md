# Hordicus

**Hordicus** is a native Windows x64 dark-fantasy horde-survival game currently in public playtesting.

## Current public playtest

**Hordicus v23.8.51 — Public Playtest 1**

[Download the latest Hordicus release](https://github.com/Sponsor-Sorter/Hordicus/releases/latest)

### Install

1. Download `Hordicus_v23_8_51_Public_Playtest_1_Windows_x64.zip` from the Releases page.
2. Extract the **entire ZIP** to its own folder.
3. Do not run the game from inside the ZIP.
4. Run `Hordicus.exe`.

No installer is required.

## File integrity

Public ZIP SHA-256:

```text
936d2124905f1c864cf7d9a6e14a14d1ec94b7c8d011f79814d7f6256ce5257a
```

`Hordicus.exe` SHA-256:

```text
b760afe5428fa53c6d2e521503fa0224a7bc5143b786fe960c4697ee9cb4666d
```

If Windows security software flags a download, verify that the filename and SHA-256 match the official release before taking any action.

## Public-build contents

The public playtest package contains the **player runtime only**.

It intentionally does **not** contain:

- Hordicus source code
- the Hordicus Map Builder / map editor
- internal development tools
- build scripts or compiler files
- internal test suites
- authoring metadata

## Feedback

Hordicus is in active development. Please use the official Hordicus Discord community for:

- bug reports
- technical support
- feature requests
- balance feedback
- multiplayer/LFG
- development updates

When reporting a bug, include your Hordicus version, Windows version, reproduction steps, expected result, actual result, and screenshots/video when useful.

## Public playtest notes

See [Public Playtest 1 notes](docs/PUBLIC_PLAYTEST_1.md) for release details and areas we want players to test.

## Platform

- Windows x64
- Extract-and-run ZIP distribution

---

Hordicus is currently under active development. Public-playtest content, balance, features and save compatibility may change between builds.
